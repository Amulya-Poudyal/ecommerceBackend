...
import { carts, cart_items, product_variants } from '../db/schema.ts';
...
export const addToCart = async (req, res) => {
    ...
    if (existingItem.length > 0) {
        const updated = await db.update(cart_items)
    ...
    const item = await db.insert(cart_items)
        .values({
            cart_id: cart.id,
            product_id,
            variant_id,
